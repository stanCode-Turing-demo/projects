...
This program adds and arranges new data into the 'name_data' dictionary.
It finally looks like {year:{name:rank}}.
"""
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            line = line.split(',')
            # Check if line is the year.
            if len(line) != 3:
                year = line[0].strip()
            else:
                rank = line[0].strip()
                name1 = line[1].strip()  # Baby boy's name
                name2 = line[2].strip()  # Baby girl's name
